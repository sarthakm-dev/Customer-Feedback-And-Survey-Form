...
export function validateRadios(requiredRadios) {
    let valid = true;
...
export function validateCurrentStep(currentStep, ratings, supportContacted) {
    let valid = true;
    if(currentStep===0){
        const orderInput = document.querySelector(`input[name="product-name"]`);
        if(!orderInput.value.trim()){
            orderInput.classList.add("invalid");
            orderInput.nextElementSibling.style.visibility = "visible";
            valid = false;
        }
...
export function validateFormSubmission(form, ratings, supportContacted) {
    let valid = true;
...
export function createFormData(form, ratings) {
